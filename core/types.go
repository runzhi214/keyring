// Package core defines the shared types and interface for the keyring
// abstraction. It is imported by the top-level keyring package (which
// re-exports everything and provides Open), by each platform backend
// (keyctl, secretservice, darwin, windows), and by the in-memory store.
//
// Splitting the types into a separate package breaks what would otherwise
// be an import cycle: the top-level keyring package imports backends, and
// backends import the types.
package core
